...
// read-data capture regs (1-cycle staging)
    ...
    localparam [2:0]
        ...
        S_LOAD_WAIT = 3'd2,   // NEW: capture read data
    ...
    // Trigger edge detect (2-FF sync + edge)
    ...
    always @(posedge clk or negedge reset_n) begin
        ...
        if (!reset_n)
            ...
            led <= 1'b0;          // clear error

        else if (invalid_trig)

            led <= 1'b1;          // latch error
    ...
    //   CHANGE: store init values as BYTES, not variable part-selects
    ...
    always @* begin
        ...
        case (state)
            ...
            S_LOAD_SET:
                ...
                nstate = S_LOAD_WAIT;    // NEW



            S_LOAD_WAIT:

                nstate = S_LOAD_CAP;     // NEW
    ...
    // Helper: pack byte arrays into 64-bit (pure wiring, no mux)
    ...
    function automatic [63:0] pack8;
        ...
        begin
            ...
            // blk0 = LSB (little-endian), blk7 = MSB
    ...
    always @(posedge clk or negedge reset_n) begin
        ...
        if (!reset_n) begin
            ...
            case (state)
                ...
                // NEW: capture read data (separates routing/reads from writes)
                ...
                // Write captured byte into the init byte memories, then advance
                ...
                S_ARMED: begin
                    ...
                    if (valid_trig_in_armed) begin
                        ...
                        for (ch = 0; ch < CHANNELS; ch = ch + 1) begin
                            ...
                            // pack 8 bytes -> 64-bit once at arm time
                            ...
                            // decide phase based on whether delay/width are zero

                            // (use the packed values implicitly via byte checks, avoids big compares)
